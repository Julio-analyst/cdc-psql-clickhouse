    ...
    try {
        ...
        if ($runningContainers) {
            ...
            if ($cdcContainers.Count -gt 0) {
                $statsCmd = 'docker stats --no-stream --format "table {{.Name}}\t{{.CPUPerc}}\t{{.MemUsage}}\t{{.NetIO}}\t{{.BlockIO}}" ' + ($cdcContainers -join " ")
...
function Get-ExistingCustomers {
    try {
        ...
        $result = docker exec postgres-source psql -U postgres -d inventory -c "SELECT id FROM inventory.customers;" 2>$null
        $customerIds = @()
...
function Get-ExistingProducts {
    try {
        ...
        $result = docker exec postgres-source psql -U postgres -d inventory -c "SELECT id FROM inventory.products;" 2>$null
        $productIds = @()
        $result -split "`n" | ForEach-Object {
            if ($_ -match '^\s*(\d+)\s*$') {
...
function Get-CurrentOrderCount {
    try {
        $result = docker exec postgres-source psql -U postgres -d inventory -c "SELECT COUNT(*) FROM inventory.orders;" 2>$null
...
"@
        try {
            $result = docker exec postgres-source psql -U postgres -d inventory -c $insertQuery 2>$null
